...
func take_damage(damage, direction, knockback_val):
	...
	mov_speed = default_speed * 0.3
	knockback_counter = knockback_recovery
...
func execute_attack():
	if in_attack_range && target != null:
		target.take_damage(enemy_attack_strength)
	else:
...
func set_data(i_health : float, i_walk : int, i_run : int, i_speedup : int):
	...
	match Globals.dificuldade:
		"facil":
			mov_speed = default_speed * 0.3
		"medio":
			mov_speed = default_speed * 0.6
		"dificil":
			mov_speed = default_speed * 1.1  # ← levemente mais rápido agora
		_:
			mov_speed = default_speed * 0.5
